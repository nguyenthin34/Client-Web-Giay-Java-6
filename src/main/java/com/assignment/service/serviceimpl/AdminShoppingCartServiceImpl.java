...
import com.assignment.service.AdminShoppingCartService;
...
public class AdminShoppingCartServiceImpl implements AdminShoppingCartService {
       ...
       if(map.size() <= 0){
           ...
           if(item == null){
               for(int i = 0; i < map.size(); i++){
                   if(cartItem.getCommodityId() == list.get(i).getCommodityId()
                           && cartItem.getColorId() == list.get(i).getColorId()
                           && cartItem.getSizeId() == list.get(i).getSizeId()
                           && cartItem.getUnitPrice() == list.get(i).getUnitPrice()){
    ...
    @Override
	public void update(Integer id, Integer quantity, Double unitPrice, Integer colorId, Integer sizeId){
